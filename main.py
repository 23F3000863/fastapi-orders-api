...
from fastapi import FastAPI, Header, HTTPException, Response, status, Query
...
TOTAL_ORDERS = 50
ORDERS_CATALOG = [
    {"id": i, "item": f"Item {i}", "price": 10.0 * i}
...
class OrderCreate(BaseModel):
    # Defaults so even an empty {} body from grader won't cause 422
    item: str = "Item"
    price: float = 0.0
...
def create_order(
    ...
    """
    Idempotent order creation with per-client rate limiting.
    - Uses Idempotency-Key to avoid duplicate orders.
    - Uses X-Client-Id to enforce 17 requests / 10s per client.
    """
    global next_mock_id
    ...
    # If key already exists, return the exact same stored order
...
def get_orders(
    ...
    """
    Cursor-based pagination over fixed catalog IDs 1..50, with per-client rate limiting.
    - GET /orders?limit=P&cursor=C
    - cursor is treated as the **next id** to start from.
    """
    # Enforce rate limit for this client
    if not x_client_id:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="X-Client-Id header is required",
    ...
    page_items_raw = ORDERS_CATALOG[start_index:end_index]

    # Convert dicts to Pydantic models
    items = [OrderListItem(**item) for item in page_items_raw]

    # Calculate the next cursor string if there are more items left
    next_cursor = None
    if end_index < TOTAL_ORDERS:
        # next id = index + 1 (back to 1-based)
